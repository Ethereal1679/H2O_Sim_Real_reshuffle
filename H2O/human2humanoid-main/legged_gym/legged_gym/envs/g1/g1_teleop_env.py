...
from phc.utils.motion_lib_g1 import MotionLibG1 #zsy
# from phc.utils.motion_lib_h1 import MotionLibH1 #zsy
...
class G1TeleopRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        if not self.headless: # 是否可视化
            self.set_camera(self.cfg.viewer.pos, self.cfg.viewer.lookat)

        self._init_buffers()
        
        if self.cfg.domain_rand.motion_package_loss:
            offset=(self.env_origins + self.env_origins_init_3Doffset) #2048x3
        ...
        self.init_done = True
        # state的长度是63
        self.trajectories = torch.zeros(self.num_envs, 63 * 100).to(self.device)             # 19dof + 19dofvel + 3angular velocity + 4projectedgravity + 19last action
        self.trajectories_with_linvel = torch.zeros(self.num_envs, 66 * 100).to(self.device) # 19dof + 19dofvel + 3angular velocity + 4projectedgravity + 19last action + 3linear velocity 加上了线速度
        # 训练速度评估器
        if self.cfg.train_velocity_estimation:
            # self.velocity_estimator = VelocityEstimator(63, 512, 256, 3, 25).to(self.device)
            self.velocity_estimator = VelocityEstimatorGRU(63, 512, 3).to(self.device) # gru网络
        ...
        if self.cfg.control.action_filt:
            self.action_filter = ActionFilterButterTorch(lowcut=np.zeros(self.num_envs*self.num_actions),
                                                        ...
                                                        device=self.device)
        # 遥操作
        #print("<<<<<",self._body_list)   #['pelvis', 'left_hip_yaw_link', 'left_hip_roll_link', 'left_hip_pitch_link', 'left_knee_link', 'left_ankle_link', 'right_hip_yaw_link', 'right_hip_roll_link', 'right_hip_pitch_link', 'right_knee_link', 'right_ankle_link', 'torso_link', 'left_shoulder_pitch_link', 'left_shoulder_roll_link', 'left_shoulder_yaw_link', 'left_elbow_link', 'right_shoulder_pitch_link', 'right_shoulder_roll_link', 'right_shoulder_yaw_link', 'right_elbow_link']
        if self.cfg.motion.teleop:   
            self.extend_body_parent_ids = [15, 19]      # elbow的关节图 
            self._track_bodies_id = [self._body_list.index(body_name) for body_name in self.cfg.motion.teleop_selected_keypoints_names] # 读取特征点关节点的id  e.g.[0,4,5, 6,7,8]
            self._track_bodies_extend_id = self._track_bodies_id + [len(self._body_list), len(self._body_list) + 1]  # [0,4,5, 6,7,8] + [20,21]   
            self.extend_body_pos = torch.tensor([[0.25, 0., 0], [0.25, 0, 0]]).repeat(self.num_envs, 1, 1).to(self.device)   
            if self.cfg.motion.extend_head: # 将头部加进去
                self.extend_body_parent_ids += [0] 
                self._track_bodies_id += [len(self._body_list)]                 # [20]头的id号
                self._track_bodies_extend_id += [len(self._body_list) + 2]      # [22]头的id号
                self.extend_body_pos = torch.tensor([[0.25, 0, 0], [0.25, 0, 0], [0, 0, 0.42]]).repeat(self.num_envs, 1, 1).to(self.device)
    ...
    def step(self, actions):
        ...
        # if self.cfg.train.distill :
            if  "expert_policy" in self.__dict__:
                ...
                # 教师网络的policy，这里将普通观测值输入，得到gt_actions
                gt_actions = self.expert_policy(full_obs.to(torch.float32))
                ### 将原来的motion值暂存，之后再赋值进去：
        ...
        if self.cfg.train_velocity_estimation:
            ...
            # 创建一个布尔掩码 ready_for_train_indices，表示哪些环境的 episode 长度大于 25。这些环境的数据将被用于训练。
            self.ready_for_train_indices = self.episode_length_buf > 25

            # MLP
            # 从 trajectories 中提取满足 ready_for_train_indices 条件的数据，作为 MLP 的输入。
            train_input = self.trajectories[self.ready_for_train_indices] #3368x6300    <----   4096x6300

            # GRU
            # Reshape A into the desired shape (num_envs, 25, 63)
            # B_reshaped = train_input.reshape(train_input.shape[0], 25, 63)
            B_reshaped = train_input.reshape(train_input.shape[0], 25, 63) # 这意味着每个环境的数据被分成 25 个时间步，每个时间步有 63 个特征。
            ...
            train_input = torch.flip(B_reshaped,dims=[1]) # 逆序
            ...
            if train_input.shape[0] > 0:
                ...
                train_target = velocity[self.ready_for_train_indices].to(self.device)   # base_lin_vel

                model_output = self.velocity_estimator(train_input)                     # self.velocity_estimator = VelocityEstimator(63, 512, 256, 3, 25).to(self.device)

                loss = F.mse_loss(model_output, train_target) # 拟合速度和目标速度
    ...
    def check_termination(self):
        ...
        if self.cfg.motion.teleop:
            if self.cfg.asset.terminate_by_ref_motion_distance:
                # 设置阈值，当rigid和motion的参考位置之间的差值大于这个值的时候就会iu被reset掉
    ...
    def compute_self_and_task_obs(self, ):
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.motion.future_tracks:
                ...
                B = self.motion_ids.shape[0] # 取横坐标 4096   motion_ids:0~4096
                ...
                # 这里的None会增加新的维度
                # 我认为这里可以理解为在当前时刻episode_length_buf的基础上加上time_steps，往后预测time_steps个，比如预测3个motions
                #### motion_start_time是一个现实和仿真的延时的修正，cfg.test管理
                ...
                B = self.motion_ids.shape[0] # 4096
                motion_times = (self.episode_length_buf + 1) * self.dt + self.motion_start_times # next frames so +1
                #和episode_length_buf一起推动
                motion_res = self._get_state_from_motionlib_cache_trimesh(self.motion_ids, motion_times, offset= offset) # 汇总 motion_res = [self.motion_ids, motion_times, offset= offset]


            #print("<<<<<>>>>>++++++=",motion_res["rg_pos_t"].shape) # 4096 * 23 *3
            #import ipdb; ipdb.set_trace()
            ...
            if self.cfg.domain_rand.motion_package_loss:# ？？？？？？
                ...
                # zsy add 参考动作的root的地方
                ref_root_pos = ref_body_pos_extend[:,0,:]
        ...
        if self.cfg.motion.teleop:
            if self.cfg.motion.teleop_obs_version == 'v1':
                with torch.no_grad():
                    ...
                    root_pos = body_pos[..., 0, :] # pelvis 上半身
                    root_rot = body_rot[..., 0, :] # pelvis 上半身
                    root_vel = body_vel[:, 0, :]   # pelvis 上半身
                    ...
                    #print("v1----self_obs: ", self_obs.shape)
                    ...
                    #print("v1----task_obs: ", task_obs.shape)
                ...
                if self.cfg.asset.zero_out_far:
                    ...
                    zeros_subset = distance > close_distance # 如果大于了近距离的值
                    self.prioritize_closing = zeros_subset
                    if self.cfg.asset.zero_out_far_change_obs:# 如果改变了观测值
                ...
                if self.cfg.use_velocity_estimation:
                    ...
                    if current_obs_a.shape[0] > 0:
                        ...
                        estimate_velocity = self.velocity_estimator(self.trajectories[self.ready_for_train_indices])
                        obs[self.ready_for_train_indices,38:41] = estimate_velocity
            # zsy add 考虑sim2real无返回值的情况：用于学生网络
            ##########################################################################################
            ## NOTE: v-teleop-extend-vr-max-nolinvel_
            elif self.cfg.motion.teleop_obs_version == 'v-teleop-extend-vr-max-nolinvel-local':
                body_pos = self._rigid_body_pos
                body_rot = self._rigid_body_rot
                body_vel = self._rigid_body_vel
                body_ang_vel = self._rigid_body_ang_vel
                ...
                task_obs = compute_imitation_observations_teleop_local(root_rot, ref_root_pos, ref_rb_pos_subset, ref_body_vel_subset, body_pos_subset ,self.cfg.motion.num_traj_samples)
                if self.cfg.env.add_short_history:
                    assert self.cfg.env.short_history_length > 0
                    history_to_be_append = self.trajectories[:, 0:self.cfg.env.short_history_length*63]
                    # 63 + 18 = 81
                    # obs: 81 + 63 * 25 = 1656
                    # previ_obs: 1656 + 77 = 1733
                    obs = torch.cat([dof_pos, dof_vel, base_ang_vel, base_gravity,  # 19dim + 19dim + 3dim + 3dim 
                                                task_obs,  # 18dim   
                                                self.actions, #19dim 
                                                history_to_be_append], dim = -1) # 63 * 25 
                    # import ipdb; ipdb.set_trace()
            ...
            ##########################################################################################
                ...
                ref_rb_pos_subset = ref_body_pos_extend[:, self._track_bodies_extend_id]
                ref_rb_rot_subset = ref_body_rot_extend[:, self._track_bodies_extend_id]
                ref_body_vel_subset = ref_body_vel_extend[:, self._track_bodies_extend_id] ## [4096, 23, 3]     # [num_envs, num_markers, 3]
                ...
                if self.cfg.asset.zero_out_far: # ref_rb_pos_subset[0], ref_rb_pos_subset[1], head ref_rb_pos_subset[2]
                    ...
                    distance = torch.norm(root_pos - ref_body_pos_extend[0::self.cfg.motion.num_traj_samples,0, :], dim=-1)
                    zeros_subset = distance > close_distance
                    #print("&&&&&&&&&&&&&&&&&&&&&",zeros_subset, torch.where(zeros_subset == 1))
                    self.prioritize_closing = zeros_subset 
                    if self.cfg.asset.zero_out_far_change_obs:
                        if self.cfg.motion.future_tracks:
                            n = self.cfg.motion.num_traj_samples
                            # import ipdb; ipdb.set_trace()
                            zeros_set_future = zeros_subset.repeat_interleave(n) # zeros_set_future[n * i: n * i + n] = zeros_subset
                            #print("@@@@@@@@@@@@@@@@@@@@zeros_set_future",zeros_set_future.shape) # 4096
                            # print(ref_rb_pos_subset[zeros_set_future, :2])
                            body_pos = body_pos_subset[zeros_subset, :2] # two hands\ # [3800, 23, 3 ]
                            # ref_rb_pos_subset [4096,23,3]
                            ref_rb_pos_subset[zeros_set_future, :2] = body_pos.repeat_interleave(n,dim=0) # [3800*2, 23, 3 ]
                            # print(ref_rb_pos_subset[zeros_set_future, :2])
                            root_vel_ = root_vel[zeros_subset].unsqueeze(1).repeat(1, 23, 1)
                            ref_body_vel_subset[zeros_set_future, :] = root_vel_.repeat_interleave(n,dim=0)
                            self.point_goal= distance
                            far_distance = self.cfg.asset.far_distance  # does not seem to need this in particular...
                            vector_zero_subset = distance > far_distance  # > 5 meters, it become just a direction
                            ...
                            ref_rb_pos_subset[zeros_subset, :2] = body_pos_subset[zeros_subset, :2] # two hands\
                            #print("&&&&&&&&&&&&&&&&&&&&&",ref_body_vel_subset.shape, root_vel.shape)
                            ref_body_vel_subset[zeros_subset, :] = root_vel[zeros_subset].unsqueeze(1).repeat(1, 23, 1) # torch.Size([4096, 23, 3])    torch.Size([4096, 3])
                            self.point_goal= distance
                            far_distance = self.cfg.asset.far_distance  # does not seem to need this in particular...
                            vector_zero_subset = distance > far_distance  # > 5 meters, it become just a direction
                            ref_rb_pos_subset[vector_zero_subset, 2] = ((ref_rb_pos_subset[vector_zero_subset, 2] - body_pos_subset[vector_zero_subset, 2]) / distance[vector_zero_subset, None] * far_distance) + body_pos_subset[vector_zero_subset, 2]
    ...
    def _init_height_points(self):
       ...
       self.num_height_points = grid_x.numel()   #返回了tensor的元素的总个数
    ...
    def _get_heights(self, position = None, env_ids=None):
        ...
        points += self.terrain.cfg.border_size #25
        points = (points / self.terrain.cfg.horizontal_scale).long() #0.1
        px = points[:, :, 0].view(-1)
        py = points[:, :, 1].view(-1)
        px = torch.clip(px, 0, self.height_samples.shape[0] - 2)
        py = torch.clip(py, 0, self.height_samples.shape[1] - 2)

        # height1 height2 height3三者的最小值
            ...
            if self.cfg.motion.teleop:
                if self.cfg.motion.realtime_vr_keypoints:
                    ...
                    for pos_id, pos_joint in enumerate(self.marker_coords[env_id]): # idx 0 torso (duplicate with 11)
                        # （0.3,0.3,0.3） 
                        color_inner = (0, 0, 1) if not self.cfg.motion.visualize_config.customize_color \
    ...
    def _process_rigid_shape_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_friction:
            ...
            # print("------------------len props",len(props)) 37
            # 由于G1机器人和H1的link数目不同，导致读出来的数据不同，这里需要修正下。。。
            for s in range(self.num_bodies):#(len(props)):
    ...
    def _process_dof_props(self, props, env_id):
        ...
        if env_id==0:  
            ...
            for i in range(len(props)):
                ...
                self.dof_pos_limits[i, 0] = m - 0.5 * r * self.cfg.rewards.soft_dof_pos_limit # 0.85的小系数，用来作一个soft max limit zsy
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_base_com:
            torso_index = self._body_list.index("waist_yaw_link")
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.domain_rand.motion_package_loss and  (self.common_step_counter % self.cfg.domain_rand.package_loss_interval == 0):
            self._freeze_ref_motion() # 隔一定时间保存一次motions的模型
    ...
    def _compute_torques(self, actions):
        ...
        # print("||",actions_scaled + self.default_dof_pos)
        control_type = self.cfg.control.control_type
        if control_type=="P":   # 位置
    ...
    def update_average_episode_length(self, env_ids):
        ...
        codff = (num / self.num_compute_average_epl) # zsy
        self.average_episode_length = self.average_episode_length * (1 - codff) + current_average_episode_length * codff
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.motion.teleop_obs_version == 'v1':
                ...
                if PLAY_SHAPE:
                    print(f"<<<<<{self.cfg.motion.teleop_obs_version}>>>>>noise_vec.shape: ", noise_vec.shape)
            elif self.cfg.motion.teleop_obs_version == 'v-min':
                ...
                max_num_bodies = len(self.cfg.motion.teleop_selected_keypoints_names) + 3 #23
                curr_obs_len = 0
                # body_pos
                ...
                self.self_obs_size = 2*self.num_dof + 6 # 44 zsy
                # ref dof pos
                if self.cfg.motion.future_tracks:
                    noise_vec[2*self.num_dof + 9 : 2*self.num_dof + 9 + (len(self.cfg.motion.teleop_selected_keypoints_names) + 3) *3 * 3 * self.cfg.motion.num_traj_samples ] = noise_scales.ref_body_pos * noise_level * self.obs_scales.body_pos 
                else:
            ...
            # NOTE: zsy added 用于vr设备的观测，学生网络 
                ...
                self.self_obs_size = 2*self.num_dof + 6             # 44 zsy
                # ref dof pos 也就是task_obs
                if self.cfg.motion.future_tracks: 
                    noise_vec[2*self.num_dof + 9 : 2*self.num_dof + 9 + (len(self.cfg.motion.teleop_selected_keypoints_names) + 3) * 3 * 2 * self.cfg.motion.num_traj_samples ] = noise_scales.ref_body_pos * noise_level * self.obs_scales.body_pos 
                else:  
                    noise_vec[2*self.num_dof + 9 : 2*self.num_dof + 9 + (len(self.cfg.motion.teleop_selected_keypoints_names) + 3) * 3 * 2] = noise_scales.ref_body_pos * noise_level * self.obs_scales.body_pos    
                if PLAY_SHAPE:
                    print(f"<<<<<{self.cfg.motion.teleop_obs_version}>>>>>noise_vec.shape: ", noise_vec.shape)
    ...
    def _init_buffers(self):
        ...
        # added by zsy
        # 1. actor_root_state = self.gym.acquire_actor_root_state_tensor(self.sim) 
        #                                |
        #                                V
        # 2. self.root_states = gymtorch.wrap_tensor(actor_root_state) 
        #                                |
        #                                V
        # 3. self.base_quat = self.root_states[:, 3:7]
        ...
        self.gravity_vec = to_torch(get_axis_params(-1., self.up_axis_idx), device=self.device).repeat((self.num_envs, 1)) #++++++ tensor([[ 0.,  0., -1.]], device='cuda:0')
        ...
        for i in range(self.num_dofs):
            ...
            # print(f"init_buffer:default_dof_pos[{i}]为：{name}\n") # 打印关节的名字
        ...
        if self.cfg.motion.teleop:
            ...
            self.motion_ids = torch.arange(self.num_envs).to(self.device) # 这行代码的作用是创建一个从 0 到 self.num_envs - 1 的整数张量
    ...
    def _create_envs(self):
        ...
        """
        #asset_path = os.path.join(LEGGED_GYM_ROOT_DIR,self.cfg.asset.file)
        ...
        self.num_dof = self.gym.get_asset_dof_count(robot_asset)  # 19
        #print("===========num_dof=============\n",self.num_dof,"\n") #
        self.num_bodies = self.gym.get_asset_rigid_body_count(robot_asset) # 20
        #print("===========num_bodies=============\n",self.num_bodies,"\n") #
        dof_props_asset = self.gym.get_asset_dof_properties(robot_asset) #19
        #print(f"=====================Number of dof_props_asset in the asset: {len(dof_props_asset)}")
        dof_props_asset["driveMode"] = gymapi.DOF_MODE_EFFORT
        
        rigid_shape_props_asset_buf = self.gym.get_asset_rigid_shape_properties(robot_asset) # 37
        rigid_shape_props_asset = self.gym.get_asset_rigid_shape_properties(robot_asset) # 37

        index = 0
        A = [0,  2,3,4,5,6,  8,9,10,11,12,  14,   24,25,26,27,   32,33,34,35]       #   在g1的一些关节的索引
        for i in A:
            rigid_shape_props_asset[index] = rigid_shape_props_asset_buf[i]
            index = index + 1
        #print("===========rigid_shape_props_asset=============\n",len(rigid_shape_props_asset),"\n") #
        ...
        for i in range(self.num_envs):
            ...
            rigid_shape_props = self._process_rigid_shape_props(rigid_shape_props_asset, i) #37
    ...
    def _get_env_origins(self):
        ...
        if self.cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            max_init_level = self.cfg.terrain.max_init_terrain_level # level
            ...
            num_cols = np.floor(np.sqrt(self.num_envs)) #sqrt(2048)
        ...
        # print("<<<<<>>>>>++++=env_origin [0]",self.env_origins[0,:])
    ...
    def _load_motion(self):
        # motion文件安
        #print("[][][][][]",self.cfg.motion.motion_file)
    ...
    def _get_state_from_motionlib_cache_trimesh(self, motion_ids, motion_times, offset=None):
        #print("<<<<<=OFFSET",offset.shape,offset) # torch.Size([2048, 3]) 
        ## Cache the motion + offset
        # import ipdb; ipdb.set_trace()
        # 当没有值的时候，正好一定是大于零的，只有当存入了才表现出等于0
        if offset is None  or not "motion_ids" in self.ref_motion_cache or self.ref_motion_cache['offset'] is None or len(self.ref_motion_cache['motion_ids']) != len(motion_ids) or len(self.ref_motion_cache['offset']) != len(offset) \
            or  (self.ref_motion_cache['motion_ids'] - motion_ids).abs().sum() + (self.ref_motion_cache['motion_times'] - motion_times).abs().sum() + (self.ref_motion_cache['offset'] - offset).abs().sum() > 0 :
            # 防止覆盖操作
            self.ref_motion_cache['motion_ids'] = motion_ids.clone()  # need to clone; otherwise will be overriden
            ...
            return self.ref_motion_cache
        motion_res = self._motion_lib.get_motion_state(motion_ids, motion_times, offset=offset) # "root_pos", "root_rot", "root_vel", "root_ang_vel"。。。。。
    ...
    def feet_distance(self):
        left_foot_pos = self._get_rigid_body_pos("left_ankle_pitch_link") #zsy
...
# ---------------------------------------------------------------------- 计算观测值 ------------------------------------------------------------------------------------------
# local是做一个torch_utils.my_quat_rotate(heading_inv_rot     的四元数将向量的乘积的映射
@torch.jit.script
# [NOTE]返回值的obs为：root坐标系下参考动作的位置和角度 & root坐标系下速度差和角速度差   & root坐标系下参考动作和机器人本体dof_pos差和dof_rot差   & root坐标系下关节速度差和关节角速度差    
def compute_imitation_observations(root_pos, root_rot, body_pos, body_rot, root_vel, root_ang_vel, dof_pos, dof_vel, ref_body_pos, ref_body_rot, ref_root_vel, ref_root_ang_vel, ref_dof_pos, ref_dof_vel, time_steps):
    ...
    diff_local_root_ang_vel = torch_utils.my_quat_rotate(heading_inv_rot.view(-1, 4), diff_global_root_ang_vel.view(-1, 3)) # 将diff_global_root_ang_vel映射到heading里面
...
def compute_imitation_observations_teleop_local(root_rot ,ref_root_pos, ref_body_pos, ref_body_vel, body_pos, time_steps):
    obs = []
    ...
    local_delta_ref_body_pos = torch_utils.my_quat_rotate(heading_inv_rot_expand.view(-1, 4), delta_ref_body_pos.view(-1, 3))
    local_delta_ref_body_vel = torch_utils.my_quat_rotate(heading_inv_rot_expand.view(-1, 4), ref_body_vel.view(-1, 3))
    # make some changes to how futures are appended.
    obs.append(local_delta_ref_body_pos.view(B, time_steps, -1))  # 1 * timestep * J * 3
    obs.append(local_delta_ref_body_vel.view(B, time_steps, -1))  # timestep  * J * 3 
...
# @torch.jit.script
# [NOTE]返回值的obs为：root坐标系下参考动作的位置 & root坐标系下参考动作和机器人动作pos之差 & （root坐标系下参考动作的速度）   ref_vel_in_task_obs默认是true
def compute_imitation_observations_teleop_max(root_pos, root_rot, body_pos,   ref_body_pos, ref_body_vel, time_steps,  ref_episodic_offset = None, ref_vel_in_task_obs = True):
    ...
    ##### body pos + Dof_pos This part will have proper futuers.
    # 广播i机制，会将1维拓展到多维从而进行像素级的运算：
    # c = torch.zeros_like(root_pos.view(B, 1, 1, 3))
    # c[...,2] = 0.3
    # c = ref_body_pos.view(B, time_steps, J, 3)
    local_ref_body_pos = ref_body_pos.view(B, time_steps, J, 3) - root_pos.view(B, 1, 1, 3)  # preserves the body position
    local_ref_body_pos = torch_utils.my_quat_rotate(heading_inv_rot_expand.view(-1, 4), local_ref_body_pos.view(-1, 3))
    ...
    # make some changes to how futures are appended.
    ### 这里的返回值是 num * 3 ，也就是把每一个追踪的Link的坐标都拼接到一起了，，，，，
    a  = diff_local_body_pos_flat.view(B, time_steps, -1) #torch.Size([4096, 1, 69]) 23 * 3 
    obs.append(a)  # 1 * timestep * J * 3
    b = local_ref_body_pos.view(B, time_steps, -1)
    obs.append(b)  # timestep  * J * 3
...
# @torch.jit.script
# [NOTE]返回值的obs为：root坐标系下参考动作的位置 & root坐标系下参考动作和机器人动作pos之差 & root坐标系下heading朝向之差 & （root坐标系下参考动作的速度）
...
# @torch.jit.script
# [NOTE]返回值的obs为：root坐标系下参考动作的位置和角度 & root坐标系下参考动作和机器人动作pos之差和rot之差 & （root坐标系下参考动作的速度和角速度）
...
# @torch.jit.script
# [NOTE]返回值的obs为：root坐标系下机器人本体动作的位置和角度 & （root坐标系下机器人本体的速度和角速度）& 机器人本体的关节速度和关节角速度
def compute_humanoid_observations(body_pos, body_rot, root_vel, root_ang_vel, dof_pos, dof_vel, local_root_obs, root_height_obs):
    ...
    root_pos = body_pos[:, 0, :] #少了一个维度 假设num_envs = 1024
    root_rot = body_rot[:, 0, :]

    root_h = root_pos[:, 2:3] # pelvis高度
    heading_rot_inv = torch_utils.calc_heading_quat_inv(root_rot)

    # 如果是false就将root_h_obs清零
    if (not root_height_obs):
        root_h_obs = torch.zeros_like(root_h)
    else:
        root_h_obs = root_h

    heading_rot_inv_expand = heading_rot_inv.unsqueeze(-2) #少了第二个维度，加上
    heading_rot_inv_expand = heading_rot_inv_expand.repeat((1, body_pos.shape[1], 1)) # repeat((1,20,1))
    ...
    root_pos_expand = root_pos.unsqueeze(-2) # 他是几维的？？？答：dim = num_envs  x 1 x 3 所以拓展出来一维
    local_body_pos = body_pos - root_pos_expand #每个位置相对于pelvis的位置   dim = num_envs  x 20 x 3
    flat_local_body_pos = local_body_pos.reshape(local_body_pos.shape[0] * local_body_pos.shape[1], local_body_pos.shape[2]) # 减去中间1维  dim = 20480  x 3
    flat_local_body_pos = torch_utils.my_quat_rotate(flat_expanded_heading_rot_inv, flat_local_body_pos) #相对相对坐标系的旋转角度
    local_body_pos = flat_local_body_pos.reshape(local_body_pos.shape[0], local_body_pos.shape[1] * local_body_pos.shape[2])
    local_body_pos = local_body_pos[..., 3:]  # remove root pos

    flat_body_rot = body_rot.reshape(body_rot.shape[0] * body_rot.shape[1], body_rot.shape[2])  # This is global rotation of the body  dim = 20480  x 4
    ...
    local_body_vel      = torch_utils.my_quat_rotate(heading_rot_inv, root_vel) #  1x3, root 计算local分量
    local_body_ang_vel  = torch_utils.my_quat_rotate(heading_rot_inv, root_ang_vel) # 1x3, root 计算local分量
...
def compute_humanoid_observations_max_full(body_pos, body_rot, body_vel, body_ang_vel,  local_root_obs, root_height_obs):
    ...
    root_pos = body_pos[:, 0, :] # 2048 x 3
    root_rot = body_rot[:, 0, :]
    

    root_h = root_pos[:, 2:3]
    heading_rot_inv = torch_utils.calc_heading_quat_inv(root_rot) #2048 x 4
    ...
    # print("\n>>>>>++heading_rot_inv>>>>>+++",heading_rot_inv.shape) 
    # import ipdb; ipdb.set_trace()
    heading_rot_inv_expand = heading_rot_inv_expand.repeat((1, body_pos.shape[1], 1)) # 2048 x 23 x 4 把中间一的维度扩展为23维，也就是重复23次
    # print("\n>>>>>++heading_rot_inv_expand>>>>>+++",heading_rot_inv_expand.shape)
    # import ipdb; ipdb.set_trace()
    flat_heading_rot_inv = heading_rot_inv_expand.reshape(heading_rot_inv_expand.shape[0] * heading_rot_inv_expand.shape[1], heading_rot_inv_expand.shape[2]) # 2048*23 x 4
    # print("\n>>>>>++flat_heading_rot_inv>>>>>+++",flat_heading_rot_inv.shape)
    # import ipdb; ipdb.set_trace()

    root_pos_expand = root_pos.unsqueeze(-2)
    local_body_pos = body_pos - root_pos_expand # 2048 x 23 x 3
    flat_local_body_pos = local_body_pos.reshape(local_body_pos.shape[0] * local_body_pos.shape[1], local_body_pos.shape[2]) # 2048*23 x 3
    flat_local_body_pos = torch_utils.my_quat_rotate(flat_heading_rot_inv, flat_local_body_pos) # 2048*23 x 3
    local_body_pos = flat_local_body_pos.reshape(local_body_pos.shape[0], local_body_pos.shape[1] * local_body_pos.shape[2]) # 2048 x 69
    local_body_pos = local_body_pos[..., 3:]  # remove root pos      # 2048 x 66
    # print("\n>>>>>++local_body_pos>>>>>+++",local_body_pos.shape)
    # import ipdb; ipdb.set_trace()


    flat_body_rot = body_rot.reshape(body_rot.shape[0] * body_rot.shape[1], body_rot.shape[2])  # This is global rotation of the body     # 2048*23 x 4
    flat_local_body_rot = quat_mul(flat_heading_rot_inv, flat_body_rot)  # 2048*23 x 4
    # print("\n>>>>>++flat_local_body_rot>>>>>+++",flat_local_body_rot.shape)
    # import ipdb; ipdb.set_trace()
    flat_local_body_rot_obs = torch_utils.quat_to_tan_norm(flat_local_body_rot) # 2048*23 x 6 切向量法向量拼接所以是6 [1,0,0] [0,0,1]
    # print("\n>>>>>++flat_local_body_rot_obs>>>>>+++",flat_local_body_rot_obs.shape)
    # import ipdb; ipdb.set_trace()
    local_body_rot_obs = flat_local_body_rot_obs.reshape(body_rot.shape[0], body_rot.shape[1] * flat_local_body_rot_obs.shape[1]) # 2048 x 23*6
    # print("\n>>>>>++local_body_rot_obs>>>>>+++",local_body_rot_obs.shape)
    # import ipdb; ipdb.set_trace()
    ...
    flat_body_vel = body_vel.reshape(body_vel.shape[0] * body_vel.shape[1], body_vel.shape[2]) # 2048*23 x 3
    # print("\n<<<<<++++++",body_vel.shape)
    # import ipdb; ipdb.set_trace()
    flat_local_body_vel = torch_utils.my_quat_rotate(flat_heading_rot_inv, flat_body_vel) # 2048*23 x 3
    # print("\n<<<<<++++++",flat_local_body_vel.shape)
    # import ipdb; ipdb.set_trace()
    local_body_vel = flat_local_body_vel.reshape(body_vel.shape[0], body_vel.shape[1] * body_vel.shape[2]) #  2048 x 69
    # print("\n<<<<<++++++",local_body_vel.shape) # torch.Size([2048, 69])
    # import ipdb; ipdb.set_trace()
