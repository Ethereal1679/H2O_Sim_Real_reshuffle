...
class MotionLibG1(MotionLibBase):
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1, target_heading = None):
        ...
        self.curr_motion_keys = self._motion_data_keys[sample_idxes]            #0-ACCAD_Male2MartialArtsPunches_c3d_E9 - body hook left_poses'
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        n = len(motion_ids) 
        num_bodies = self._get_num_bodies() # 20
        ...
        body_vel0 = self.gvs[f0l] # 全局线速度
        body_vel1 = self.gvs[f1l]

        body_ang_vel0 = self.gavs[f0l] # 全局角速度
        body_ang_vel1 = self.gavs[f1l]

        rg_pos0 = self.gts[f0l, :] # 全局位移
        rg_pos1 = self.gts[f1l, :]

        dof_vel0 = self.dvs[f0l] # 关节速度
        dof_vel1 = self.dvs[f1l]
        # 汇总
        vals = [local_rot0, local_rot1, body_vel0, body_vel1, body_ang_vel0, body_ang_vel1, rg_pos0, rg_pos1, dof_vel0, dof_vel1]
        for v in vals:
            # 这段代码的作用是遍历 vals 中的每个元素 v，并断言每个元素的数据类型不是 torch.float64（即双精度浮点数）。如果任何一个元素的数据类型是 torch.float64，则会触发断言错误并抛出异常。
        ...
        rb_rot0 = self.grs[f0l] # global rotations zsy
        rb_rot1 = self.grs[f1l]
        rb_rot = torch_utils.slerp(rb_rot0, rb_rot1, blend_exp)
        return_dict = {}
        
        if "gts_t" in self.__dict__:
            rg_pos_t0 = self.gts_t[f0l] # global translations zsy
        ...
        return_dict.update({
            ...
            "motion_aa": self._motion_aa[f0l], # 运动数据中的轴角表示（Axis-Angle），用于描述旋转，形状取决于帧数。
            "rg_pos": rg_pos, 
            "rb_rot": rb_rot,
            "body_vel": body_vel,
