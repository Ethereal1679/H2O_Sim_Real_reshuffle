...
import os
# added by zsy
MY_ROOT_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))))
print("\\\\\\\\\\\\\\\\\\\\\\",MY_ROOT_DIR)
# 根据G1需要更改
...
# zsy:好像只需要更改mjcf_file路径就可以了
class Humanoid_Batch:
    def __init__(self, mjcf_file = f"{MY_ROOT_DIR}/resources/robots/g1/g1.xml", extend_hand = True, extend_head = False, device = torch.device("cpu")):
        print("&&&&&&&&&",mjcf_file)
        self.mjcf_data = mjcf_data = self.from_mjcf(mjcf_file)
        #print(self.mjcf_data)
