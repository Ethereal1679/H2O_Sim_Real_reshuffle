    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:  
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % cfg.sim_config.decimation == 0: 
                ...
                obs[19*2+6   :     19*3+6]           = np.copy(target_actions )       #19 
                obs[19*3+6   :     19*3+24]          = np.copy(task_obs )     #18 
                obs[19*3+24  :     19*3+24+63*25]    = np.copy(trajectories[0 : cfg.env.frame_stack * cfg.env.num_single_obs]) 
                ...
                current_obs_a = np.copy(obs[0:63])
